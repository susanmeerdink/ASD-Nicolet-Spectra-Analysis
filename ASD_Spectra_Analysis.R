### ASD_Spectra_Analysis ###
## Susan Meerdink
## 10/14/2016
## This code reads in ASD spectra processed by ASD_Data_Prep.R 
...
dataFile <- "C:\\Users\\Susan\\Documents\\GitHub\\ASD-Nicolet-Spectra-Analysis\\ASD_Averaged_Spectra_Huntington_rep3.csv" #Set to spectra file location
metaFile <- "C:\\Users\\Susan\\Documents\\GitHub\\ASD-Nicolet-Spectra-Analysis\\ASD_Averaged_Spectra_Metadata_Huntington_rep3.csv" #Set to metadata file location
...
## WARNING: will produce 2151 image files!
...
for(x in c(2:ncol(spectra))){#Loop through wavelengths
  ...
  ggplot(data = plotData,aes(wl))+ ##Plotting Histogram
    geom_histogram(binwidth = .01) + #plot as histogram
    ggtitle(wavelength[x-1]) + #add title to histogram
    xlab("Reflectance (%)") #add xlabel
  
  plotName <- paste(directory,"Normality_Test_ASD\\",round(wavelength[x-1]*1000,digits = 0),"_hist.png",sep="") #create plot name to save the file
  ggsave(plotName,width = 3,height = 3) #save the plot
}
fileName <- paste(directory,"Normality_Test_ASD\\asd_normality_test_results.csv",sep="") 
...
fileNameKW <- paste(directory,"Kruskal_Wallis_Test_ASD\\asd_kruskal_wallis_test_results.csv",sep="") #create filename that will hold kw test results
write("Wavelength,KW_pvalue",file = fileNameKW)#write header to file
fileNameKWPH <- paste(directory,"Kruskal_Wallis_Test_ASD\\asd_kruskal_wallis_posthoc_test_results.csv",sep="") #create filename that will hold kw post hoc test results
write("Wavelength,Pair,Obs-Diff,Critical-Diff,Flag",file = fileNameKWPH)
fileNameKWDT <- paste(directory,"Kruskal_Wallis_Test_ASD\\asd_dunn_posthoc_test_results.csv",sep="") #create filename that will hold kw post hoc test results
...
dunnFile <- "C:\\Users\\Susan\\Documents\\GitHub\\ASD-Nicolet-Spectra-Analysis\\Kruskal_Wallis_Test_ASD\\asd_dunn_posthoc_test_results.csv" #Set to metadata file location
...
ggplot(data = dunnResults, aes(dunnResults$Wavelength)) + #Plot the data
  geom_histogram(binwidth = .01, fill = "gray",col = "black",boundary = 0) + #Plot it as a Histogram 
  theme_set(theme_bw(base_size=20)) + #Set the theme 
  scale_y_continuous(expand = c(0, 0), limits = c(0, 2000)) + #removing offset on y axis
  scale_x_continuous(breaks = seq(0.3,2.5,0.1)) + #changing tick mark frequency in x axis
  labs(x = expression(paste("Wavelength (",mu,"m)")), y = "Frequency") #Add labels to plot
plotName = paste(directory,"Kruskal_Wallis_Test_ASD\\asd_Pair_Wavelength_Frequency.png",sep="") #create plot name to save the file
